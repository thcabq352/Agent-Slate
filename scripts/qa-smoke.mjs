...
const outDir = process.env.QA_OUT || resolve(root, 'docs/images')
...
const steps = [
  wait(700),
  clickContains('.home-project', 'Night Market'), wait(800),
  { shot: 'qa-1-editor-specbar' },
  clickText('.tab', 'Deliver'), wait(500),
  js(`const sel=document.querySelector('.scroll select'); if(sel){sel.value='minimax-h3'; sel.dispatchEvent(new Event('change',{bubbles:true}))}`),
  wait(700),
  { shot: 'qa-2-deliver-h3' },
  js(`const sel=document.querySelector('.scroll select'); if(sel){sel.value='seedance-2'; sel.dispatchEvent(new Event('change',{bubbles:true}))}`),
  wait(400),
  { shot: 'qa-3-deliver-seedance' },
  clickText('.tab', 'Refs'), wait(500),
  { shot: 'qa-4-refs-stills' },
  js(`[...document.querySelectorAll('button')].find(b=>b.textContent.includes('Scan dailies'))?.click()`),
  wait(1200),
  { shot: 'qa-5-stills-scanned' }
]
